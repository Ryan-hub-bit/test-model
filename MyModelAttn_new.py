...
def init_dataset(Revedges = True, Adddata = True, Addfunc = True, DataRefedgs = True, Calledges = True, CodeRefedgs = True, Laplacian_pe = False):
    dataset = iCalldsAttn.iCallds2(Revedges=Revedges, Calledges=Calledges, Laplacian_pe=Laplacian_pe,
...
def get_one_graph(dataset, i, Adddata = True, Addfunc = True, Laplacian_pe=False):
    ...
    g = g.to(device)
    if Adddata:
        print(f'{i} Code: {g.num_nodes("code")}, Data: {g.num_nodes("data")}, Edges: {g.num_edges()}, GT: {glabels["GT_edges"].shape[1]}, GT_F_edges:{glabels["GT_F_edges"].shape[1]}')
        node_features = {
            'code': g.nodes['code'].data['feat'].view(g.nodes['code'].data['feat'].shape[0], -1).float(),
            'data': g.nodes['data'].data['feat'].view(g.nodes['data'].data['feat'].shape[0], -1).float()}
    else:
        print(f'{i} Code: {g.num_nodes("code")}, Edges: {g.num_edges()}, GT_edges: {glabels["GT_edges"].shape[1]}, GT_F_edges:{glabels["GT_F_edges"].shape[1]}')
        node_features = {
            'code': g.nodes['code'].data['feat'].view(g.nodes['code'].data['feat'].shape[0], -1).float()}
    if Addfunc:
        if Laplacian_pe:
...
# def main():
    ...
    savedir = "/home/isec/test/model" + "/result_attn" +"_" + str(inslen)
    exp_all(epochs = epochs, hidden_features=hidden_features, inslen = inslen,  savePATH = savedir + '/allpe',
            Revedges = True, Adddata = True, Addfunc = True, DataRefedgs = True, Calledges = False, CodeRefedgs = True, Laplacian_pe=True)
