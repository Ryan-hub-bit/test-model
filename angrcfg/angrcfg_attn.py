...
graphdir = "/home/isec/Documents/attncall/graph_dir_new"
...
for root, dirs, files in os.walk(binfolder):
    for binf in files:
        ...
        #         if call_target not in addrtoid:
        #             print(f"callee_address{call_target} not in addrtoid")
        #             output += f"callee_address:{call_target}  not found"
        #         else:
        #             callee_id = addrtoid[call_target]
        #             if  isinstance(nodelookup[call_target],datatrainnode):
        #                 output += "datanode"
        #             elif isinstance(nodelookup[call_target],codetrainnode):
        #                 codecnt += 1
        #                 output += "codenode"

        #             output += f"callee_addr:{call_target}"
        #         if call_site not in addrtoid:
        #             print(f"callsite_address{call_site} not in addrtoid")
        #             output += f"callee_address:{call_target}  not found"
        #         else:
        #             callsite_id = addrtoid[call_site]
        #             output += f"callsite_addr:{call_site}"
        ...
        #                         f.write(output)
        # with open(os.path.join(directory, "cnt.txt"), 'a') as f:
        #     cntstr = f"{binfile} : {codecnt}"
        #     f.write(cntstr)
                    ...
                    #print(f"insn: {insn}, type: {insn.operands[0].type} \n")
                        ...
                        #print(f"{str(insn)}, {typeofins}")
                            ...
                            if callsite_id != 0 and callee_id != 0:
                                ...
                                cnt += 1
                                GT_edges.append(new_edge)
        with open(os.path.join(directory, "original_cnt.txt"), 'a') as f:
            original_cnt = f"{binfile} : {cnt}"
